...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    document.title = `Search Results for "${query}" | LocalMarket`;
    
    // In a real app, this would be an API call
    const searchListings = () => {
      setIsLoading(true);
      
      // Filter listings by search query and location
      const results = listings.filter(listing => {
        ...
        return matchesQuery && matchesLocation && matchesCategory && matchesPrice;
      });
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">{searchResults.length} listings found</p>
          </div>
                  ...
                  <div key={cat.id} className="flex items-center">
                    ...
                    >
                      <div className={`w-5 h-5 rounded border flex items-center justify-center ${
                        selectedCategory === cat.id ? 'bg-teal-600 border-teal-600' : 'border-gray-300'
                      }`}>
